...
def kdd_search(nounEntityList, user_input_text, gec_message):     
    ...
    try:
        ...
        if connection.is_connected():
           ...
           queryResultList=[]
           if (user_input_text !=''):
               mysqlstatement = 'SELECT id, response_count, tag_id, question, response_message, keywords, message_type FROM knowledgebase WHERE question like\'%'+gec_message+'%\' '
...
def sqlforMinimizeRecords(nounEntityList, no_duplicate, dbcursor):
    ...
    for entity in nounEntityList:
        mysqlstatement = 'SELECT id, response_count, tag_id, question, response_message, keywords, message_type FROM knowledgebase WHERE question like\'%'+entity+'%\' OR keywords like\'%'+entity+'%\' HAVING question like\'%'+nounEntityList[0]+'%\' AND question like\'%'+nounEntityList[1]+'%\'  '
        dbcursor.execute(mysqlstatement)
        qres = dbcursor.fetchall()
